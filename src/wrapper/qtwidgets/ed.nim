...
proc comment*(this:var string; txt:string; symbol:string = "#") =
  var n = 1
  var tmp = ""
  var limits = this.find(txt)
  var lines = this[0..<limits].splitLines
  var a = lines.len
  var b = a + txt.splitLines.len - 1
  this.comment( a, b, symbol )

proc commentIF*(this:var string; txt:string; symbol:string = "#") =
  var n = this.find(&"#if {txt}")
  #echo ">>",n
  var m = this.find("#endif", n + txt.len)
  echo n, " ", m
  this.comment( this[0..<n].splitLines.len, 
                this[0..<m].splitLines.len,
                symbol )
  # var lines = this[0..<limits].splitLines
  # var a = lines.len
  # var b = a + txt.splitLines.len - 1
...
proc comment*(header:var Header; txt:string) =
  header.txt.comment(txt, "//")

proc commentIF*(header:var Header; txt:string) =
  header.txt.commentIF(txt, "//")
